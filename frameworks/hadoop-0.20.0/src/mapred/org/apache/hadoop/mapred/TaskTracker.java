...
public class TaskTracker 
  ...
  // Current numbers of map slots and reduce slots
  volatile int mapSlots;
  volatile int reduceSlots;
   ...
   */
  volatile int heartbeatInterval = HEARTBEAT_INTERVAL_MIN;
  /**
  ...
  synchronized void initialize() throws IOException {
    ...
    System.out.println("Created job client: " + jobClient);
  ...
  public TaskTracker(JobConf conf, NexusExecutor nexusExecutor)
    ...
    mapSlots = maxCurrentMapTasks;
    reduceSlots = maxCurrentReduceTasks;
  ...
  public void killTask(TaskAttemptID taskAttemptId) {
    LOG.info("Asked to kill task: " + taskAttemptId);
    TaskInProgress tip = null;
    synchronized (this) {
      tip = tasks.get(taskAttemptId);
    }
    try {
      purgeTask(tip, false);
    } catch (IOException e) {
      LOG.error("IOException in purgeTask", e);
    }
  }
