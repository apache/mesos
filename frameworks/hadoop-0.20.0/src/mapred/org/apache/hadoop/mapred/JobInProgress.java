...
class JobInProgress {
  ...
  private boolean taskCachesAvailable = false;
  ...
  synchronized boolean hasMapToLaunch(String host, int maxCacheLevel) {
    // For scheduling a map task, we have two caches and a list (optional)
    //  I)   one for non-running task
    //  II)  one for running task (this is for handling speculation)
    //  III) a list of TIPs that have empty locations (e.g., dummy splits),
    ...
    if (!mapCleanupTasks.isEmpty()) return true;
    
    // Return false right away if the task caches aren't ready, either because
    // we are still initializing or because we are cleaning up
    if (!taskCachesAvailable) return false;
    ...
    // 3. Search non-local tips for a new task
    if (hasUnlaunchedTask(nonLocalMaps))
      return true;
    ...
    if (hasSpeculativeMaps) {
      ...
      float avgProg = status.mapProgress();
      ...
      // 3. Check non-local tips for speculation
      for (TaskInProgress tip: nonLocalRunningMaps) {
        if (tip.isRunning() && tip.hasSpeculativeTask(time, avgProg)) {
          return true;
        }
      }
  ...
  synchronized boolean hasReduceToLaunch() {
    // Return false if not enough maps have finished to launch reduces
    if (!scheduleReduces()) return false;
    
    // Check for a reduce cleanup task
    if (!reduceCleanupTasks.isEmpty()) return true;
    ...
    if (nonRunningReduces.size() > 0) return true;
    
    // check for a reduce to be speculated
    if (hasSpeculativeReduces) {
      long time = System.currentTimeMillis();
      float avgProg = status.reduceProgress();
  ...
  private boolean hasUnlaunchedTask(Collection<TaskInProgress> cache) {
    if (cache != null)
      for (TaskInProgress tip: cache)
        if (tip.isRunnable() && !tip.isRunning())
          return true;
