...
TEST(CgroupDeviceAccessTest, IsAccessGrantedTest)
{
  ...
  cgroup_device_access = CHECK_NOTERROR(DeviceManager::CgroupDeviceAccess::create(
  ...
  cgroup_device_access.allow_list.push_back(*devices::Entry::parse("c 1:3 rw"));
...
TEST(CgroupDeviceAccessTest, NormalizedTest)
{
  ...
  // Not normalized if there are entries which are encompassed by another on the
  // same list
  cgroup_device_access = DeviceManager::CgroupDeviceAccess::create(
