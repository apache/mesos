...
{
public:
  LinuxLauncherProcess(const Flags& flags, const CgroupsInfo& cgroupsInfo);
...
Try<Launcher*> LinuxLauncher::create(const Flags& flags)
{
  ...
  if (mounted.isError()) {
    return Error(
        "Failed to check if cgroups v2 is mounted: " + mounted.error());
  }
  if (*mounted) {
...
bool LinuxLauncher::available()
{
  ...
  Try<bool> mounted = cgroups2::mounted();
  available |= mounted.isSome() && mounted.get();
#endif // ENABLE_CGROUPS_V2
  ...
  available |= ::geteuid() == 0 && freezer.isSome() && freezer.get();

  return available;
}


LinuxLauncher::LinuxLauncher(const Flags& flags, const CgroupsInfo& cgroupsInfo)
  : process(new LinuxLauncherProcess(flags, cgroupsInfo))
{
    ...
    const vector<ContainerState>& states)
{
  ...
  // filesystem.
  Try<Nothing> recover = Nothing();
  if (cgroupsInfo.cgroupsV2) {
    recover = recoverContainersFromCgroups2();
  } else {
    recover = recoverContainersFromCgroups();
  }

  if (recover.isError()) {
    return Failure(
        "Failed to recover containers from the cgroup filesystem: " +
        recover.error());
  }
...
Try<Nothing> LinuxLauncherProcess::recoverContainersFromCgroups()
{
  ...
  Try<vector<string>> freezerCgroups =
    cgroups::get(cgroupsInfo.freezerHierarchy.get(), flags.cgroups_root);

  if (freezerCgroups.isError()) {
    return Error(
        "Failed to get cgroups from " +
        path::join(cgroupsInfo.freezerHierarchy.get(), flags.cgroups_root) +
  ...
  if (cgroupsInfo.systemdHierarchy.isSome()) {
    Try<vector<string>> systemdCgroups =
      cgroups::get(cgroupsInfo.systemdHierarchy.get(), flags.cgroups_root);

    if (systemdCgroups.isError()) {
      return Error(
          "Failed to get cgroups from " +
          path::join(cgroupsInfo.systemdHierarchy.get(), flags.cgroups_root) +
...
Try<Nothing> LinuxLauncherProcess::recoverContainersFromCgroups2()
{
  ...
  return Nothing();
#else
  return Error("Cannot call 'recoverContainersFromCgroups2' if cgroups "
               "v2 is not enabled");
#endif // ENABLE_CGROUPS_V2
}
    ...
    const vector<int_fd>& whitelistFds)
{
  ...
  // Ensure we didn't pass `enterNamespaces` if we aren't forking a nested
  // container.
  ...
  Try<Subprocess> child = subprocess(
    ...
    [parentPid, enterFlags, cloneFlags](const lambda::function<int()>& child) {
      if (parentPid.isSome()) {
        Try<pid_t> pid =
          ns::clone(parentPid.get(), enterFlags, child, cloneFlags);
        if (pid.isError()) {
...
Future<Nothing> LinuxLauncherProcess::destroy(const ContainerID& containerId)
{
  ...
  if (cgroupsInfo.cgroupsV2) {
    return destroyCgroups2(container.get());
  } else {
    return destroyCgroups(container.get());
  }
}
  ...
  const Container& container)
{
  ...
  return Nothing();
#else
  return Error("Cannot call 'destroyCgroups2' if cgroups v2 is not enabled");
#endif // ENABLE_CGROUPS_V2
}
