    ...
    const ContainerID& containerId)
{
  ...
  // 3. Each of the requested controllers are enabled => Log
  //
  ...
  const string nonLeafCgroup =
    cgroups2_paths::container(flags.cgroups_root, containerId);
  ...
  if (!cgroups2::exists(nonLeafCgroup)) {
    ...
    Try<Nothing> createNonLeaf = cgroups2::create(nonLeafCgroup);
    if (createNonLeaf.isError()) {
      return Failure("Failed to create cgroup '" + nonLeafCgroup + "': "
                     + createNonLeaf.error());
    }
  }
  ...
  if (!cgroups2::exists(leafCgroup)) {
    ...
    Try<Nothing> createLeaf = cgroups2::create(leafCgroup);
    if (createLeaf.isError()) {
      return Failure("Failed to create cgroup '" + leafCgroup + "': "
                     + createLeaf.error());
    }
  }
  ...
  const Try<set<string>> enabledControllers = cgroups2::controllers::enabled(
      nonLeafCgroup);
  if (enabledControllers.isError()) {
    return Failure(
        "Failed to get the enabled controllers for container "
        "'" + stringify(containerId) + "': " + enabledControllers.error());
  }
    ...
    pid_t pid)
{
  ...
  // Move the process into its leaf cgroup. Since info->cgroup is the non-leaf
  // cgroup, we get the corresponding leaf cgroup by appending /leaf. It's
