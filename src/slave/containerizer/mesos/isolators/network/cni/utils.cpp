  ...
  for (int i = 0; i < labels->labels().size(); i++) {
    ...
    if (label->has_value()) {
      ...
      if (value.length() > 63) {
        ...
        // Keep the first 7 characters of the MD5 sum and the last 55 characters
        // of the label, concatenating them with a tilde (63 chars in total)
        size_t ofs = value.length() - 55;
